...
// #######################################################
const descPing = [
    "Use Adobe's content management system, AEM, to provide the structure for team members to quickly design, publish \
        and modify web content on the companies main website. Well-versed with the technologies for constructing and \
        maintaining a robust, scalable web presence at: \
    ...
    `Cloud computing work: Independently built AWS Lambda Functions, AWS Api Gateways, CloudWatch services, \
        an updated to our Cloudfront CDN, and the accompanying GitLab-CI pipeline in an effort to migrate \
        endpoints from our CMS to AWS. I held responsiblity for the architecure, pipeline, IaC tool, and building it \
        all from start to finish.`,
    `Wrote the Terraform configurations for the services and wrote the NodeJS code of the Lambda Functions \
        for the migrated endpoint. There have not been any records of them failing since they were built. \
        These endpoints are vital and cannot fail.`,
    ...
    'Frontend work: building web components for content and design teams, enabling static \
        and dynamic page functionalities across the website. Advanced in  JavaScript, HTML, and CSS.',
...
const descCharter  = [
    ...
    "Routinely accessed virtual machines (VMs) via SSH to execute and develop various scripts. \
        Use command-line tools to navigate the internal file system of the VMs to locate log files, \
        view, filter, and analyze logs for troubleshooting and performance monitoring",
    'Development a Flask/Python-based proxy server to streamline traffic \
        and facilitate data retrieval from a MongoDB database.',
    "Wrote a Python-based testing tool for quality assurance of a DHCP server cluster, utilizing \
    ...
    "Deployed docker apps, running Telegraf, to gather metrics from the VMs running the DHCP servers. \
        It streamed data into our Splunk server for real-time data analysis. I also created a Splunk dashboard to visualize and examine the data.",
...
const Jobs3 = [jobPing, jobCharter]
const Projects3 = [projAndriod, projVFO,  projMV, projResume, projGit ]
module.exports = { Jobs3, Projects3 };
